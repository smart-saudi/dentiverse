...
| Preview smoke validation      | Blocked              | Preview deploy `dpl_9mrkoSB8wceBsVjiw4EaUjBGLetv` reached `Ready`, and `/`, `/login`, and `/designers` returned healthy shells via `vercel curl`, but `/api/v1/designers` failed with `{\"code\":\"INTERNAL_ERROR\",\"message\":\"TypeError: fetch failed\"}` because hosted runtime secrets still point at placeholder or local-only services |
...
- PR validation note: PR #3 confirmed `Validate App` passes in GitHub Actions. The only remaining CI issue found during LR-09 was Terraform formatting drift in [infra/terraform/main.tf](../../../infra/terraform/main.tf), which was fixed on the follow-up branch commit after the first PR run.
