...
- **M9-LR-10** Final launch sign-off and doc lock â€” _Blocked until Vercel runtime secrets point to real cloud services and hosted preview smoke passes on data-backed routes_
...
- **M9-LR-10** Final launch sign-off and doc lock once real hosted runtime config is in place and preview smoke passes end-to-end.
...
- [x] **M9-LR-06** Define and implement the admin operating model - 2026-03-24
- [x] **M9-LR-07** Implement transactional email - 2026-03-24
...
| 2026-03-24 | #24 | Completed `LR-06` admin operating model. Chose the controlled-launch manual-ops path, documented operator roles and reversible procedures for support, moderation, dispute handling, and refunds in a dedicated operations doc, updated the runbook, and replaced the architecture-level “Admin Panel” promise with the real manual-ops model. Verified `npm.cmd run check`, `npm.cmd test` (`328` passing), `npm.cmd run test:e2e` (`11` Chromium specs), and `npm.cmd run build`. | Start `LR-07` for transactional email |
| 2026-03-24 | #25 | Completed `LR-07` transactional email. Added `EmailService` around Resend, wired proposal-received and design-submitted emails from the case routes, wired payment-confirmed and payment-released emails from Stripe webhooks with duplicate-send guards based on Stripe IDs, documented sender env vars and disabled-email fallback behavior, and added unit/integration coverage for the new event hooks. Verified `npm.cmd run check`, `npm.cmd test` (`334` passing), `npm.cmd run test:e2e` (`11` Chromium specs), and `npm.cmd run build`. | Start `LR-08` auth-scope decision |
| 2026-03-24 | #26 | Completed `LR-08` auth-scope alignment. Formalized the v1 launch decision to ship email/password plus password reset only, de-scoped Google OAuth and Magic Link from launch, and updated contributor docs, security docs, the system architecture diagram, and the launch tracker so they match the actual runtime auth surface. Verified `npm.cmd run check`, `npm.cmd test` (`334` passing), `npm.cmd run test:e2e` (`11` Chromium specs), and `npm.cmd run build`. | Start `LR-09` preview smoke tests and rollback rehearsal |
| 2026-03-24 | #27 | Completed `LR-09` preview and rollback rehearsal. Fixed the invalid deploy workflow conditions, added archive-mode prebuilt deploys for Vercel, linked the repo to the `dentiverse` Vercel project, created preview and production GitHub environments plus deploy secrets, and rehearsed real preview and production deploys. Preview page shells passed, but data-backed hosted routes still fail until Vercel runtime secrets are replaced with real cloud values. Production rollback back to `dpl_C7R7GM1wSWdqaT2KFQ8pekPLmUKh` succeeded in about `6.66s`. | Start `LR-10` with real hosted runtime config and final sign-off |
...
- [ ] **M9-LR-10** Final launch sign-off and doc lock after real hosted runtime config is in place and preview smoke passes on data-backed routes
...
| --- | -------- | ------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Decision | Auth provider                                                                                                 | Supabase Auth (email/password + password reset for v1 launch). Google OAuth and Magic Link are intentionally de-scoped until a post-launch product decision reintroduces them.                                                                                                                  |
...
| 11  | Issue    | Launch readiness is still incomplete                                                                          | `npm run check`, `npm test`, `npm run test:e2e`, and `npm run build` are green, preview and rollback rehearsal are now documented, but final sign-off is still blocked on replacing placeholder Vercel runtime secrets with real cloud values and re-running hosted smoke on data-backed routes |
