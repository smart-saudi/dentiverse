...
- **M9-LR-09** Run preview smoke tests and rollback rehearsal once the remaining launch docs and flows are aligned.
...
- [ ] **M9-LR-09** Run preview smoke tests and rollback rehearsal and then continue the remaining launch blockers through `M9-LR-10`
...
| 11  | Issue    | Launch readiness is still incomplete                                                                          | `npm run check`, `npm test`, `npm run test:e2e`, and `npm run build` are green, auth abuse protection is in place, observability wiring is active, transactional email is wired, the admin operating model is documented, launch auth scope is aligned to email/password-only, and the next blocker is `M9-LR-09` in `docs/phase-5/operations/LAUNCH_BACKLOG.md` |
